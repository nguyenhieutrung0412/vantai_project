...
class bTemplate extends CaoBox{
	...
	function getTemplate($tpl_dir,$currentLang){
		...
		if(count($this->files)) foreach($this->files as $block=>$array){
			...
			if($mytpl==''){
				...
				$mytpl = str_replace('{include:tpl='.$block.'}',$content,$mytpl);
			}
		}
	...
	function parse($mimeType = 'text/html'){
		...
		foreach($this->files as $arr){
			...
			$t = filemtime($arr['dir'].$arr['file']);
			if($t > $max_time) $max_time = $t;
		}
		$this->cachefile = 'tpl_'.md5($file).'.php';
		//echo $this->cache;
		//echo filemtime($this->cache.$this->cachefile);
	...
	function cache(){
		if($this->cache && $this->cachefile && !file_exists($this->cache. $this->cachefile)){
