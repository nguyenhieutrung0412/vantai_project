...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($count_insert > 0) {
        foreach ($_REQUEST['data'] as $key => $list) {
            ...
            $model->db->query("INSERT INTO php_mathang_donhang  (`id_donhang`, `ten`, `dvt`, `soluong`, `ghichu`) VALUES ('" . $id_donhang . "','" . $name . "','" . $dvt . "','" . $soluong . "','" . $ghichu . "')");
        }
    }
    ...
    if ($count_update > 0) {
        foreach ($_REQUEST['update'] as $key => $list) {
            ...
            $model->db->query("UPDATE `php_mathang_donhang` SET `ten`='" . $name . "',`dvt`='" . $dvt . "',`soluong`='" . $soluong . "',`ghichu`='" . $ghichu . "' WHERE id ='" . $id . "'");
        }
    }
