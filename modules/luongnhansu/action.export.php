...
if($_GET['thang'] != ''|| $_GET['nam'] != ''){
    ...
    $list ='';
    $list_luong =  $oContent->view_table(" php_luongnhansu"," active = 1 AND thang = ".$_GET['thang']." AND nam = ".$_GET['nam']);
    ...
    while($rs = $list_luong->fetch()){
        ...
        $name_nhansu = $model->db->query("SELECT * FROM php_nhansu WHERE id =  ".$rs['user_id']);
        ...
        $list .='
            <tr>
                ...
                <td style=" height:30px;text-align:center;">'.$rs2['name'].'</td>
    ...
    <div class="table"> 
        <table border = "1">
            <thead>
                <tr>
                    <th colspan = "12"><h3 style="color: red;">Bảng lương nhân sự tháng '.$_GET['thang'].' năm '.$_GET['nam'].'<h3></th>
                </tr>
                <tr>
    ...
    ';
        ...
        $file_name = 'bang-luong-'.$_GET['thang'].'-'.$_GET['nam'];
