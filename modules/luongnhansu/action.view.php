...
if($_SESSION['chucvu_id'] == 0 ||$_SESSION['chucvu_id'] == 1 || $_SESSION['chucvu_id'] == 2)
{
	...
	if($_GET['code'] == '' &&$_GET['thang'] == 0 &&$_GET['nam'] == 0 )
	{
		...
		$thang_nam = $model->db->query("SELECT thang,nam FROM php_luongnhansu GROUP BY id DESC LIMIT 1 ");
		...
		//Phân trang
		$phieu_chi = $oContent->view_table('php_luongnhansu',"thang=".$thang_and_nam['thang']." AND nam=".$thang_and_nam['nam']);
		...
		$rs2['tongluong_thang'] = number_format($rs2['tongluong_thang'], 0, ',', '.') . "";
			$tpl->merge($rs2,'tong');
		}
	}
	else{
		...
		$rs2['nam'] = $_GET['nam'];
			...
			//Phân trang
			$phieu_chi_search =  $model->db->query("SELECT * FROM php_luongnhansu WHERE active < 5".$id_where.$thang_where.$nam_where);
			//lấy tổng số phần tử tin 
			$total = $phieu_chi_search->num_rows();
			if($total >= 1) {
				$limit = 10;
				$start = $limit*intval($_GET['page']);
				$url = $system->root_dir.'luongnhansu';
				$dp = new paging($url,$total,$limit);
				$dp->current = '<a class="active_page">%d</a>';
				$tpl->assign(array('divpage'=>$dp->simple(10)));
		
		
				$phieu_chi_search = $oContent->view_pagination('php_luongnhansu',"active < 5".$id_where.$thang_where.$nam_where."  ORDER BY id DESC",$start,$limit);
				...
				header("Location: ".$domain."luongnhansu");
				exit;
			}
	...
	header("Location: ".$domain."luongnhansu/v/?code=".$id_security);
	exit;
}
