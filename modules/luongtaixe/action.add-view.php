...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $module = "'luongtaixe'";
    $add = "'add'";
    $frm = "'frmAddluongtaixe'";
    $img_file = 1;
    $rs['id_security'] = $oClass->id_encode($rs['id']);

    $result3 = $oContent->view_table("php_taixe");
    while($rs3 = $result3->fetch()){
        $nhansu .= ' <option value="'.$rs3['id'].'" >'.$rs3['name_taixe'].'</option>';
    }

    $listnhansu = "'listtaixe'";
    $idSelectThang = "'thang'";
    $idSelectNam = "'nam'";
    for($i = 1;$i<=12;$i++)
    {
        ...
        <div class="pop-up">
            <h3>Thêm mới</h3>
            <form autocomplete="off" name="frmAddluongtaixe" id="frmAddluongtaixe" method="post" onsubmit = "return add('.$module.','.$add.','.$frm.',1)"  enctype="multipart/form-data">
