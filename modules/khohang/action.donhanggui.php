...
$tpl->setfile(array(
	...
	'tpl_body' => 'truckho_donhanggui_detail.tpl',
...
if (isset($_GET['code'])) {
	...
	$don_hang = $oContent->view_pagination('php_donhangroi_s', "kho_guihang = " . $id . "  GROUP BY id DESC", $start, $limit);
	$stt = 1;
	while ($rs = $don_hang->fetch()) {
		...
		{
			$rs['tinhtrang_kho'] = 'Xác nhận đã giao';
			$rs['disalbed'] = '';
			$rs['link'] = "OrderConfirmation_gui('khohang','xacnhandonhanggui',".$rs["id_security"].")";
		}
		else if($rs['tinhtrang_khogui'] == 1)
		{
			$rs['tinhtrang_kho'] = ' Đã giao';
		...
		{
			$rs['tinhtrang_khonhan_check'] = 'Chưa nhận';
			$rs['tinhtrang_khonhan_color'] = ' color-0';
		}
		else if($rs['tinhtrang_khonhan'] == 1)
		{
			$rs['tinhtrang_khonhan_check'] = 'Đã nhận';
			$rs['tinhtrang_khonhan_color'] = ' color-1';
		}
