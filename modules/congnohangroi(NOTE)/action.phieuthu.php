...
$id_congnohangtrongoi = $oClass->id_decode($_GET['code']);
//Phân trang
...
$url = $system->root_dir.'congnohangroi/phieuthu';
$dp = new paging($url,$total,$limit);
$dp->current = '<a class="active_page">%d</a>';
$tpl->assign(array('divpage'=>$dp->simple(10)));


$phieu_chi = $oContent->view_pagination('php_phieuthu',"id_congnohangroi = ".$id_congnohangtrongoi." AND active = 1 GROUP BY id DESC",$start,$limit);
...
while($rs = $phieu_chi->fetch()){
	...
	// format tien te
	$rs['sotien_thu'] = number_format($rs['sotien_thu'], 0, ',', '.') . "VND";
