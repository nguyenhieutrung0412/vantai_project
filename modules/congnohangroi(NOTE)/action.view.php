...
if ($_SESSION['chucvu_id'] == 0 || $_SESSION['chucvu_id'] == 1 || $_SESSION['chucvu_id'] == 2) {
	...
	if ( $_GET['thang'] == 0 && $_GET['nam'] == 0) {
		...
		$thang_nam = $model->db->query("SELECT thang,nam FROM php_congnohangroi GROUP BY id DESC LIMIT 1 ");
		...
		if ($thang_nam->num_rows() == 1) {
			...
			//Phân trang
			$congno = $oContent->view_table('php_congnohangroi', "thang=" . $thang_and_nam['thang'] . " AND nam=" . $thang_and_nam['nam'] . "  GROUP BY id DESC");
			...
			while ($rs = $congno->fetch()) {
				...
				$phieuthu_congno = $model->db->query("SELECT * FROM php_phieuthu WHERE id_congnohangroi =" . $rs['id']." AND active_congnohangroi = 1");
				$count_phieuthu = $phieuthu_congno->num_rows();
				if($count_phieuthu > 0)
				{
					$rs['so_lan_thanh_toan'] = '<a href="/congnohangroi/phieuthu/?code='.$rs['id_security'].'"><span style="color:#39c449;display:block;">(Lần '.$count_phieuthu.')</span></a>';
				}
		...
		//Phân trang
		$phieu_chi_search =  $model->db->query("SELECT * FROM php_congnohangroi WHERE active < 5" . $id_where . $thang_where . $nam_where);
		//lấy tổng số phần tử tin 
		$total = $phieu_chi_search->num_rows();
		if ($total >= 1) {
			$limit = 10;
			$start = $limit * intval($_GET['page']);
			$url = $system->root_dir . 'congnohangroi';
			$dp = new paging($url, $total, $limit);
			$dp->current = '<a class="active_page">%d</a>';
			$tpl->assign(array('divpage' => $dp->simple(10)));


			$phieu_chi_search = $oContent->view_pagination('php_congnohangroi', "active < 5" . $id_where . $thang_where . $nam_where . "  ORDER BY id DESC", $start, $limit);
