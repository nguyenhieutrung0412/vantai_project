...
{		
	if(isset($_GET['from'])  && isset($_GET['to']) ){
		...
		$where_from_to = " AND ngay_thaynhot BETWEEN '".$gio_quocte_tu."' AND '".$gio_quocte_den."'";
		//end
	}
	if(isset($_GET['taixe']))
	{
	...
	$rs2['nam'] = Date('Y');
		//Phân trang
		$kho = $oContent->view_table('php_thaynhot');
		//lấy tổng số phần tử tin 
		$total = $kho->num_rows();

		$limit = 20;
		$start = $limit*intval($_GET['page']);
		$url = $system->root_dir.'thaynhot';
		$dp = new paging($url,$total,$limit);
		$dp->current = '<a class="active_page">%d</a>';
		$tpl->assign(array('divpage'=>$dp->simple(10)));

		
		$kho = $oContent->view_pagination('php_thaynhot',"thang = ".Date('m')." AND nam= ".Date('Y').$where_from_to .$where_id_taixe .$where_id_xe .$where_id_donvi,$start,$limit);
	...
	$tpl->assign(array('divpage'=>$dp->simple(10)));
	 ...
	 $kho = $oContent->view_pagination('php_thaynhot',"thang = ".$_GET['thang']." AND nam= ".$_GET['nam'] .$where_from_to .$where_id_taixe .$where_id_xe .$where_id_donvi,$start,$limit);
