...
{		
	if(isset($_GET['from'])  && isset($_GET['to']) ){
		...
		$where_from_to = " AND ngay_suachua BETWEEN '".$gio_quocte_tu."' AND '".$gio_quocte_den."'";
		//end
	}
	if(isset($_GET['taixe']))
	{
	...
	$rs2['nam'] = Date('Y');
		//Phân trang
		$kho = $oContent->view_table('php_theodoisuachua');
		//lấy tổng số phần tử tin 
		$total = $kho->num_rows();

		$limit = 20;
		$start = $limit*intval($_GET['page']);
		$url = $system->root_dir.'theodoisuachua';
		$dp = new paging($url,$total,$limit);
		$dp->current = '<a class="active_page">%d</a>';
		$tpl->assign(array('divpage'=>$dp->simple(10)));

		
		$kho = $oContent->view_pagination('php_theodoisuachua',"thang = ".Date('m')." AND nam= ".Date('Y').$where_from_to .$where_id_taixe .$where_id_xe .$where_id_donvi,$start,$limit);
	...
	$tpl->assign(array('divpage'=>$dp->simple(10)));
	 ...
	 $kho = $oContent->view_pagination('php_theodoisuachua',"thang = ".$_GET['thang']." AND nam= ".$_GET['nam'] .$where_from_to .$where_id_taixe .$where_id_xe .$where_id_donvi,$start,$limit);
