...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    else if($_REQUEST['id_donvi'] != '')
    {  $count_xe = count($_REQUEST['id_donvi']);
        $m = 1;
        foreach($_REQUEST['id_donvi'] as $key=>$value)
        {
            ...
            if($m == $count_xe)
            {
             ...
             //lấy tên tài xế
             $taixe = $model->db->query("SELECT * FROM php_donvi_suachua where id = ".$value ." LIMIT 1");
            $rs_taixe = $taixe->fetch();
            $name .= $rs_taixe['ten_donvi'] .';';
            $m++;
        }
    }
