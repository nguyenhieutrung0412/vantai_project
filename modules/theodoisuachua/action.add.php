...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($count_insert > 0) {
        for($i = 1;$i <= $count_insert;$i++)
        {
			   ...
			   $thoigian = explode("/",$_REQUEST['data'][$i]['ngay_suachua']);
            ...
            $ngay_do = $gio_quocte ;
            $km_luc_do =  $_REQUEST['data'][$i]['km_luc_suachua'];
            ...
            $model->db->query("INSERT INTO php_theodoisuachua  (`id_taixe`,`id_doixe`,`id_donvisuachua`, `ngay_suachua`,`km_luc_suachua`, `noidung`,`tong_tien`,`ngay`,`thang`,`nam`,`active`) VALUES ('" . $id_taixe . "','" . $id_doixe . "','" . $id_donvi . "','" . $ngay_do . "','" . $km_luc_do .  "','" . $noidung . "','" . $tong_tien . "','" . $ngay . "','" . $thang . "','" . $nam . "','0')");
            $lastid = $model->db->last_insetid("php_theodoisuachua");
					...
					// foreach($_FILES['data']['name'][$i]['img_file'] as $name => $value){
						if($img_post != ''){
							...
							if(in_array($extension,$arr_img_type)){
								...
								imagedestroy($src);
								imagedestroy($tmp);
								$model->db->query("INSERT INTO php_images(`type`,`type_id`,`file_name`,`file_type`) VALUES ('php_theodoisuachua','".$lastid."','".$img_src."','".$extension."')");
							}
							$ext_file = array('pdf'); 
							...
							if(in_array($extension,$ext_file)){
								...
								$model->db->query("INSERT php_file (`type`,`type_id`,`file_name`,`file_type`) VALUES ('php_theodoisuachua','$lastid','$img_src','$extension')");
							}
